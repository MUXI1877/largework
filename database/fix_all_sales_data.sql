...
-- ============================================
-- 第一步：修复销售机会数据 (project_opportunity)
-- ============================================
...
SET 
    ...
    END,
    `expected_delivery_date` = COALESCE(`expected_delivery_date`, DATE_ADD(CURDATE(), INTERVAL 7 DAY)),
    -- 添加降库查询参数（示例数据）
    `caliber` = CASE 
...
SET 
    ...
    `expected_delivery_date` = CASE 
        WHEN `expected_delivery_date` IS NULL OR `expected_delivery_date` = '0000-00-00' OR `expected_delivery_date` = '' 
        THEN DATE_ADD(CURDATE(), INTERVAL 15 DAY)
